...
export class EmailService {
  ...
  private wrapInTemplate(title: string, content: string, subtitle?: string): string {
    return `
      <!DOCTYPE html>
      <html>
        <head>
  ...
  async sendCustomEmail(
    ...
    try {
      ...
      await this.sendEmail({ to, subject, html: wrappedHtml, text, attachments })
...
export default new EmailService(`${process.env.CLIENT_URL || 'http://localhost:3000'}`)